...
public class Ex04_try_catch {
	public static void main(String[] args) {
		// 다중 try catch 사용
		// 프록르ㅐㅁ을 구동할 때 하나의 예외만 발생한다면 어렵지 않다
		// 하지만 try구문 안에서 예외는 다양학 ㅔ발생할 수 있다
		// 만약 기존과 같은 방법으로 처리한다면 예외를 제외하고는 제대로 처리할수없다
		...
		try {
			...
			System.out.println("뽑은 카드의 번호는 " + cards[cardIndex]); // OutOfBoundsException
		} catch (InputMismatchException e) {
			System.out.println("정수를 입력해주세요. " + e);
			
		}catch (ArrayIndexOutOfBoundsException e) {
			System.out.println("해당 번호의 카드는 없습니다. " + e);
		} 
