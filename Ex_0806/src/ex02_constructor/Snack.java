...
public class Snack {
	...
	// 기본기본생성자를 자동으로 제공해주고 있다.
	// 다만, 컴파일러의 눈에만 보일 뿐 우리가 보는 코드에는 생략되어 있다.
	
	// 기본생성자
	// 매개변수가 별도로 없으며, 중괄호 블록안에 코드가 없는 비어있는 생성자를 말한다
	// 기본생성자는 개발자가 직접 선언하지 않았을 때만 컴파일러가 자동으로 추가한다.
	// 만약 개발자가 직접 생성자를 선언한다면, 컴파일러는 선언된 생성자를사용한다
//	public Snack() { 
//		
//	}
	public Snack(int p) {
	...
	// 생성자를 통해 객체를 메모리에 올림과 동시에, 더 나아가 객체 멤버의 접근이 가능하므로
	// 일반적인 메서드처럼 객체 멤버의 데이터를 초기화 할 수 있다.
	// 메서드를 통해 매개변수에 값을 전달할 수 있던것 처럼
	// 생성자 역시 매개변수를 통해 필드에 값을 전달 할 수 있다.
	void info() {
